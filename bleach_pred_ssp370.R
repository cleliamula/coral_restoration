...
for (year in 2020:2100){
  for (month in 1:12){
    ...
    anom_370<-cbind(anom_370,(1*(t_m-2)>t_month_dict[[month]]))
    sc<-sc+1
  }
  print (year)
}
...
library(ggplot2)
anom_count_370<-matrix(NA,360,720)
anom_count_370[reef_cells]<-rowSums(anom_370)
plot(raster(anom_count_370),col=plasma(100))

#first time bleaching
first_t_370<-c()
for (i in 1:nrow(anom_370)){
  first_t_370<-c(first_t_370,min(which(anom_370[i,]==1)))
}

anom_first_370<-matrix(NA,360,720)
anom_first_370[reef_cells]<-first_t_370
plot(raster(anom_first_370),col=plasma(100))
...
anom_trend_370<-apply(anom_370,1,trend) 
